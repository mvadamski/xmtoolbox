...
function xm_inbound_post(env, endpoint, subject, body) {
  // let payload = {
  //   properties: {
  //     subject : subject,
  //     body    : body
  ...
  // return create(env, `/api/integration/1/functions/${endpoint}/triggers`, body);
}
 ...
 */
async function getMany({ baseUrl, limiter, auth, subdomain, log, proxy }, api, query = {}, label = '', throwError) {
...
async function getMAX({ baseUrl, limiter, auth, subdomain, log }, api) {
...
async function statusPageGet({ baseUrl, limiter, subdomain, log }, api) {
  const available_paths = {
    unresolved_incidents: 'incidents/unresolved.json?api_key=9822a775-ff47-4d29-b302-0be1ca8f1521',
    components          : 'components.json?api_key=9822a775-ff47-4d29-b302-0be1ca8f1521',
    component_groups    : 'component-groups.json?api_key=9822a775-ff47-4d29-b302-0be1ca8f1521'
  }

  const uri     = `${baseUrl}/${available_paths[api]}`;
  const options = { method: 'GET', uri };
  const body    = await limitControl(limiter, rp, options, log);

  return body;
  // return JSON.parse(body); // Doesn't work now, already parsed in the body apparently...
}

async function statusPageDeleteComponent({ baseUrl, limiter, subdomain, log }, api) {
  const uri     = `${baseUrl}/components/${api}?api_key=9822a775-ff47-4d29-b302-0be1ca8f1521`;
 ...
 */
async function create({ baseUrl, limiter, auth, subdomain, log, readOnly, proxy }, api, json, label = '', dropId = false, throwError, method = 'POST') {
 ...
 * @name delete
 */
async function _delete({ baseUrl, limiter, auth, subdomain, log, readOnly, throwError, proxy }, api, id, label = '') {
  const uri     = `${baseUrl}${api}${id}`;
  const options = { method: 'DELETE', uri, auth, throwError, proxy, timeout: 45000 };
  
  log.debug('XMAPI:', uri, 'Delete', 'id:', id);
 ...
 */
async function update({ baseUrl, limiter, auth, subdomain, log, readOnly, throwError, proxy }, api, json, id, label = '') {
  const name    = json.targetName || json.name || '';
  const uri     = `${baseUrl}${api}`;
  json.id       = id;
  const options = { method: 'POST', uri, auth, json, throwError, proxy };
...
async function updatePUT({ baseUrl, limiter, auth, subdomain, log, readOnly, throwError, proxy }, api, json, id, label = '') {
  // const name    = json.targetName || json.name || '';
  const uri     = `${baseUrl}${api}`;
  // json.id       = id;
  const options = { method: 'PUT', uri, auth, json, throwError, proxy };
